...
for kinect_folder in "$DATASET_DIR"/*/*/*/{K,k}inect
do
  # Check if any deidentified video already exists in the GoPro folder
  if ls "$kinect_folder"/*_deidentified.mp4 1> /dev/null 2>&1; then
    pretty_print "[$(date)] Skipping Folder" "Deidentified video already exists in folder: $kinect_folder"
    continue
  fi

  # Process each video in the GoPro folder if no deidentified video is found
  for file in "$kinect_folder"/*rgb_stream.mp4
  do
    if [ -f "$file" ]; then
      pretty_print "[$(date)] Processing Video" "$file"
      filename=$(basename "$file")
      new_filename="${filename%.mp4}_deidentified.mp4"
      output_path="$kinect_folder/$new_filename"
