...
DATASET_DIR="/standard/UVA-DSA/NIST EMS Project Data/DataCollection_Spring_2025/CARS/organized/cars_1/stroke/"
# DATASET_DIR="/standard/UVA-DSA/NIST EMS Project Data/DataCollection_Spring_2025/CARS/organized"
script_path='./egoblur/EgoBlur/script/efficient_lp_blur.py'
model_path='./egoblur/EgoBlur/weights/ego_blur_lp.jit'
...
do
  ...
  # for file in "$gopro_folder"*synced_720p.mp4 # use for raw undeidentified videos
  for file in "$gopro_folder"*_gsam2_deidentified.mp4 # use for final deidentified videos to add license plate blur
  ...
  do
    if [ -f "$file" ]; then
      ...
      new_filename="${filename%.mp4}_lp_deidentified.mp4"
      ...
      python -u $script_path --lp_model_path $model_path --input_video_path "$file" --output_video_path "$output_path" --lp_model_score_threshold 0.8
