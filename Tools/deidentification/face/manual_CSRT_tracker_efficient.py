...
class FaceBlurApp(QMainWindow):
    def __init__(self):
        ...
        self.rois = []; self.start_frame = None
        # --- FIX: Store blur coordinates, not full frames ---
    ...
    def _apply_blur_to_frame(self, frame, frame_idx):
        """Helper method to apply stored blur regions to a given frame."""
        if frame_idx in self.blur_regions:
            for x, y, w, h in self.blur_regions[frame_idx]:
                ...
                if roi.size > 0: # Avoid errors on empty ROIs
    ...
    def load(self):
        ...
        self.current = 0; self.rois = []; self.start_frame = None
        self.blur_regions.clear() # --- FIX: Clear the correct data structure
        self.saved = False
        for btn in (self.play_btn, self.prev3_btn, self.prev_btn, self.next_btn, self.next3_btn, self.mark_btn, self.track_btn, self.save_btn, self.close_btn):
            btn.setEnabled(True)
        self.show_frame(0)
    ...
    def show_frame(self, idx):
        ...
        # --- FIX: Apply blur on-the-fly for display ---
        frame = self._apply_blur_to_frame(frame, idx)

            
        # --- THE FIX (Part 1) ---
        # Second, draw green boxes for any ROIs that have been marked on this
        # frame but have not yet been tracked.
        if self.rois:
    ...
    def mark_roi(self):
        ...
        # Create a temporary frame for the ROI selector that includes existing blurs and pending boxes
        ...
        if dlg.exec_() == QDialog.Accepted and dlg.rect:
            ...
            # --- THE FIX (Part 2) ---
            # Instead of redrawing the frame from scratch, just call show_frame.
            # show_frame is now the single source of truth for how a frame should look.
    ...
    def track_blur(self):
        ...
        # 1. Determine the single, continuous block of frames to process
        ...
        # 2. Open the video file once and create a list for active trackers
        cap2 = cv2.VideoCapture(self.video_path)
        cap2.set(cv2.CAP_PROP_POS_FRAMES, min_start_frame)
        
        active_trackers = [] # Will hold dicts: {'tracker': obj, 'end_frame': int}

        frame_idx = min_start_frame
        # 3. Loop through the video frames ONCE
        for frame_idx in range(min_start_frame, max_end_frame + 1):
            ret, frame = cap2.read()
            if not ret: break

            # 4. Check if we need to start any new trackers on this frame
            for start_frame, bbox in self.rois:
            ...
            # 5. Update all currently active trackers
            # We iterate over a copy so we could remove trackers if needed, though not strictly necessary here.
            for active_tracker_info in active_trackers:
                # Skip trackers that have completed their duration
        ...
        cap2.release()
        self.rois.clear() # All pending ROIs have been processed
        self.saved = False
        self.show_frame(frame_idx) # Refresh the view
    ...
    def save(self):
        # --- FIX: Check the correct data structure ---
        ...
        try:
            ...
            if 'was_cancelled' in locals() and was_cancelled and os.path.exists(path):
                try:
                    ...
                    print(f"Error removing cancelled file: {e}")

    def close_app(self):
        # --- FIX: Check the correct data structure ---
        if self.blur_regions and not self.saved:
            resp = QMessageBox.question(self, "Unsaved Changes", "You have unsaved blur regions. Save before closing?", QMessageBox.Save | QMessageBox.Discard | QMessageBox.Cancel, QMessageBox.Save)
