        ...
        for trial_data in scenario['trials']:
            ...
            for keystep in trial_data['keysteps']:
                ...
                for video_path, current_view in video_paths:
                    ...
                    output_dir = os.path.join(dataset_root,subject_id,trial_id,label)
                    ...
                    # Check if the clip already exists and size matches
                    if os.path.exists(output_clip):
                        clip_size = os.path.getsize(output_clip)
                        if clip_size > 0:
                            print(f"    [INFO] Clip already exists: {output_clip} (size: {clip_size} bytes)")
                            continue
                        else:
                            print(f"    [WARNING] Clip exists but is empty, will overwrite: {output_clip}")
