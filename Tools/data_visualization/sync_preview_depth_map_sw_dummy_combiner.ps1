...
& $ffmpegPath -i $video1 -i $video4 -i $video5 -filter_complex "[0:v]scale=640:360[v1];[1:v]scale=640:360[v4];[2:v]scale=640:360[v5];[v1][v4][v5]hstack=inputs=3[layout]" -map "[layout]" -map 0:a  -c:v libx264 -c:a aac -pix_fmt yuv420p $outputVideo
