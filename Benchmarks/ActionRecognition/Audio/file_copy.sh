...
AD_FILE_EXT="*encoded_trimmed.mp3" #MP3
AD_AVOID_EXT="*NONE*" # Kinect recordings
...
# # copy audio files
find . -type f -name "$AD_FILE_EXT" ! -name "$AD_AVOID_EXT" -print0 | rsync -avh --progress --ignore-existing --size-only --files-from=- --from0 ./ "$DEST_DIR/"
